...
VXLAN是一种通过UDP封装二层以太网帧的网络虚拟化技术，它通过在现有的IP网络上创建覆盖网络，实现了跨三层网络的二层网络扩展。Linux内核中的VXLAN实现包括报文封装、解封装、FDB表管理等核心功能，支持多种配置选项和扩展功能。
...
### 10.4.2 对称 IRB (Integrated Routing and Bridging) 配置
...
### 10.5.1 查看 BGP EVPN 路由

```bash
show bgp l2vpn evpn
```

### 10.5.2 查看 VXLAN VNI 信息

```bash
show evpn vni
```

### 10.5.3 查看 EVPN MAC 表

```bash
show evpn mac vni 10
```

### 10.5.4 查看 EVPN ARP/ND 表
...
### 11.5.1 头端复制(Head-end Replication)
...
### 11.6.1 MLAG(Multi-Chassis Link Aggregation)集成
